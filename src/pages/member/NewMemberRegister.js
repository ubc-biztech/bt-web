...
const NewMemberRegisterFormContainer = () => {
    ...
    fetchBackend(`/users/get?id=${values.id}`, 'GET')
      ...
      .then((response) => {
        if (response === "User not found.") {
          ...
          alert(`A user with the given student ID already exists!
            Double check that your student ID is correct, or ensure that you are using the same account you signed up with the first time.
            If you are still having trouble registering, contact one of our devs.`)
        }
      })
      .catch(err => {
        alert("Signup failed");
...
export default NewMemberRegisterFormContainer;